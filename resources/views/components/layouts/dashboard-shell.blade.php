    ...
    <div class="flex min-h-screen" x-data="{ sidebarOpen: true, mobileSidebar: false, profileOpen: false, bellOpen: false }" @keydown.escape.window="mobileSidebar = false; profileOpen = false; bellOpen = false">
        ...
        >
            <div class="flex h-16 items-center justify-center border-b border-blue-900 px-2">
                <a href="{{ route('dashboard') }}" class="flex items-center justify-center">
                    <img
                        src="{{ isset($settings['logo_filename']) ? asset('image/' . $settings['logo_filename']) : asset('image/logo-bimbel.png') }}"
                        alt="eBimbel"
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto p-3 text-sm">
                @include('components.layouts.sidebar-renderer')
            </nav>
        ...
        <div x-show="mobileSidebar" x-cloak class="fixed inset-0 z-50 lg:hidden">
            ...
            >
                <div class="flex items-center justify-between border-b border-blue-900 p-4">
                    <img src="{{ isset($settings['logo_filename']) ? asset('image/' . $settings['logo_filename']) : asset('image/logo-bimbel.png') }}" alt="eBimbel" class="h-9 w-auto">
        ...
        <div class="flex min-w-0 flex-1 flex-col">
            <header class="sticky top-0 z-30 border-b border-slate-200 bg-white">
                <div class="flex h-16 items-center justify-between gap-3 px-4 sm:px-6">
                    ...
                    <div class="flex shrink-0 items-center gap-1 sm:gap-2">
                        ...
                        <a href="{{ route('settings.website') }}" class="rounded-full border border-slate-200 bg-white p-2 text-slate-600 hover:bg-slate-50" aria-label="Pengaturan" title="Pengaturan Website">
                        ...
                        <div class="relative">
                            ...
                            <div x-show="profileOpen" @click.outside="profileOpen = false" x-transition x-cloak class="absolute right-0 z-40 mt-2 w-48 rounded-xl border border-slate-200 bg-white py-1 shadow-lg">
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit" class="flex items-center gap-2 w-full px-4 py-2 text-left text-sm text-rose-700 hover:bg-rose-50">
                                        <svg class="h-5 w-5 shrink-0 text-current opacity-90" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 12l-3 3m0 0l3 3m-3-3h18"/></svg>
                                        Keluar</button>
                                </form>
                            </div>
                        </div>
            ...
            <footer class="border-t border-slate-200 bg-white px-6 py-3 text-xs text-slate-500">
                &copy; {{ date('Y') }} Bimbel Jarimatrik. Lembaga Bimbingan Belajar.
            </footer>
        </div>
    </div>
</x-layouts.dashboard>
